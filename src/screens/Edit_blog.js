...
export default function Edit_blog({ blog }) {
    console.log(blog)
    const Navigate = useNavigate();
    const [input, setInput] = useState({ _id: blog._id, blog: blog.blog });
    const handleChange = (e) => {console.log(input)
        const newblog = { ...input };
    ...
    const submit = async (e) => {
        ...
        let response = await Axios.post('http://localhost:5000/edit_blog', ({ _id: input._id, blog:input.blog }));
